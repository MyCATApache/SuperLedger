...
public class ChaincodeExecutor {
    ...
    public void executeTransactionAsyn(HFClient client, Channel channel, boolean invoke, String func,Consumer<String> consumer, String... args) throws InvalidArgumentException, ProposalException, UnsupportedEncodingException, InterruptedException, ExecutionException, TimeoutException, BuzException {
        TransactionProposalRequest transactionProposalRequest = client.newTransactionProposalRequest();
        transactionProposalRequest.setChaincodeID(ccId);
        transactionProposalRequest.setChaincodeLanguage(TransactionRequest.Type.GO_LANG);
        System.out.println("args=========================>>>");
        ...
        if (ret.equals("")) throw new BuzException("申请失败");
        consumer.accept(ret);
        final String lambda_ret = ret;
        if (invoke) {
            System.out.println("Sending transaction to orderers...");
